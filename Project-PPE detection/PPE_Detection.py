...
cap = cv2.VideoCapture('../Videos/ppe-1-1.mp4') # if you have multiple webcam then you change the value of 0 to 1
frame_width = int(cap.get(3)) # getting the frame width and height
frame_height = int(cap.get(4))

out = cv2.VideoWriter('output.avi',cv2.VideoWriter_fourcc('M','J','P','G'),10,(frame_width,frame_height))
#After detection output video will save in
# output.avi file

model=YOLO("best.pt") # passing the path of algorithm
...
while True:
    ...
    # stream = Trye will use the generator and it is more efficient than normal
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            #Place the label text on top of the background using cv2.putText().
        out.write(img)      #save output in output file
        cv2.imshow("Image",img)
        #Display the frame with the added annotations.


    if cv2.waitKey(1) & 0xFF == ord('1'):
#Exit the loop and stop the program if the '1' key is pressed.
        break
out.release()
